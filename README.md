# dotfiles
Random dotfiles from me, totally not copied from other people's github

# Screenshot
![Screenshot (sway)](https://raw.githubusercontent.com/hi-im-kent/dotfiles/main/screenshot.png)
![Screenshot (hyprland)](https://raw.githubusercontent.com/hi-im-kent/dotfiles/main/screenshot2.png)

## Where these files come from 
- [waybar config](https://github.com/mverna27/dotfiles_OLD/tree/master/.config/waybar)
- [sway, mako and i3config colour](https://draculatheme.com/)
- [The Arch wiki (ofc)](https://wiki.archlinux.org/)

## I recommend these BetterDiscord plugins if you use it
- [Plugin Repo](https://betterdiscord.app/plugin/PluginRepo)
- [Theme Repo](https://betterdiscord.app/plugin/ThemeRepo)
- [Platform Indicators](https://betterdiscord.app/plugin/PlatformIndicators)
- [Auto Idle On AFK](https://betterdiscord.app/plugin/AutoIdleOnAFK)
- [Better Friend List](https://betterdiscord.app/plugin/BetterFriendList)
- [Do Not Track](https://betterdiscord.app/plugin/DoNotTrack)
- [Read All Notifications Button](https://betterdiscord.app/plugin/ReadAllNotificationsButton)
- [Spell Check](https://betterdiscord.app/plugin/SpellCheck)
- [Split Large Messages](https://betterdiscord.app/plugin/SplitLargeMessages)
- [Translator](https://betterdiscord.app/plugin/Translator)
